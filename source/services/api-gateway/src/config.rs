...
impl Config {
    pub fn from_env() -> anyhow::Result<Self> {
        ...
        Ok(Config {
            jwt_secret,
            redis_url: env::var("REDIS_URL").expect("REDIS_URL must be set in .env file"),
            approved_emails,
            environment: env::var("ENVIRONMENT").expect("ENVIRONMENT must be set in .env file"),
            ...
            totp_only_mode: env::var("TOTP_ONLY_MODE")
                ...
                .parse()
                .unwrap_or(false),
            neo4j_uri: env::var("NEO4J_URI").expect("NEO4J_URI must be set in .env file"),
            neo4j_username: env::var("NEO4J_USERNAME").unwrap_or_else(|_| {
                env::var("NEO4J_USER").expect("NEO4J_USERNAME or NEO4J_USER must be set")
            }),
            ...
            mfa_required: {
                let mfa_env =
                    env::var("MFA_REQUIRED").expect("MFA_REQUIRED must be set in .env file");
            ...
            mfa_issuer: env::var("MFA_ISSUER").expect("MFA_ISSUER must be set in .env file"),
