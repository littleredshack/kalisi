...
mod tests {
    ...
    async fn test_neo4j_connection() {
        ...
        let graph = GraphDb::new(&config).await.expect("Failed to connect to Neo4j");
    ...
    async fn test_system_overview() {
        ...
        // Get system overview
        let overview = graph.get_system_overview().await.expect("Failed to get overview");
    ...
    async fn test_track_event() {
        ...
        // Track a test event
        let event_id = graph.track_event(
            "test_event",
            "Test Event",
            "This is a test event",
            "test@example.com"
        ).await.expect("Failed to track event");
