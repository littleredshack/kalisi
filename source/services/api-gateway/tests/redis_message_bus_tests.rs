...
async fn test_redis_message_bus_communication() {
    let redis_url =
        std::env::var("TEST_DATABASE_URL").unwrap_or_else(|_| "redis://127.0.0.1:6379".to_string());
    ...
    let _: () = redis
        ...
        .await
        .expect("Failed to add request to stream");

    // Verify request was added
    let entries: redis::streams::StreamReadReply = redis
        .xread(&["agent:requests"], &["0"])
        .await
        .expect("Failed to read requests stream");
    assert!(
        !entries.keys.is_empty(),
        "No entries found in agent:requests stream"
    );
    ...
    let data = entry
        .map
        .get("data")
        .expect("No data field in request entry");
    ...
    if let redis::Value::BulkString(bytes) = data {
        ...
        let parsed_request: serde_json::Value =
            serde_json::from_str(&json_str).expect("Invalid JSON in request data");
...
async fn test_api_gateway_agent_communication() {
    ...
    let _: () = redis
        .xadd("agent:requests", "*", &[("data", test_data)])
        .await
        .expect("Failed to create agent:requests stream");
    let _: () = redis
        .xadd("agent:responses", "*", &[("data", test_data)])
        .await
        .expect("Failed to create agent:responses stream");

    // Verify streams exist and have data
    let requests: redis::streams::StreamReadReply = redis
        .xread(&["agent:requests"], &["0"])
        .await
        .expect("Failed to read requests");
    let responses: redis::streams::StreamReadReply = redis
        .xread(&["agent:responses"], &["0"])
        .await
        .expect("Failed to read responses");

    assert!(
        !requests.keys.is_empty(),
        "agent:requests stream not created"
    );
    assert!(
        !responses.keys.is_empty(),
        "agent:responses stream not created"
    );
}
...
async fn test_log_query_processing() {
    let redis_url =
        std::env::var("TEST_DATABASE_URL").unwrap_or_else(|_| "redis://127.0.0.1:6379".to_string());
    let mut agent = SecurityAgent::new(&redis_url)
        .await
        .expect("Failed to create Security Agent");
    agent
        .initialize()
        .await
        .expect("Failed to initialize Security Agent");
    ...
    let _: () = redis
        .lpush("logs:all", log_json)
        .await
        .expect("Failed to add test log");

    // Process log query
    let response = agent
        .process_query("show me the logs")
        .await
        .expect("Failed to process log query");
    ...
    let entries: redis::streams::StreamReadReply = redis
        .xread(&["agent:activities"], &["0"])
        .await
        .expect("Failed to read agent activities");
    ...
    assert!(
        stream.ids.len() >= 5,
        "Expected multiple MRAP phases, found {}",
        stream.ids.len()
    );

    // Verify activities were also logged to main logs for visibility
    let main_logs: Vec<String> = redis
        .lrange("logs:all", 0, -1)
        .await
        .expect("Failed to read main logs");
    let agent_logs: Vec<_> = main_logs
        .iter()
    ...
    assert!(
        !agent_logs.is_empty(),
        "Security Agent activities not found in main logs"
    );
}
...
async fn test_agent_error_handling() {
    ...
    let response1 = agent
        .process_query("")
        .await
        .expect("Failed to process empty query");
    assert!(!response1.summary.is_empty());

    let response2 = agent
        .process_query("nonexistent category logs")
        .await
        .expect("Failed to process nonexistent category");
