...
impl AgentLogger for RedisAgentLogger {
    ...
    async fn publish_log_event(&mut self, agent_id: String, log_data: &Value) -> Result<()> {
        ...
        let _: () = self
            .redis_connection
            .publish("logs:stream", &log_json)
            .await?;

        // Agent-specific stream
        let _: () = self
            .redis_connection
            .publish(&format!("logs:agent:{}", agent_id), &log_json)
            .await?;
        ...
        if let Some(category) = log_data.get("category").and_then(|c| c.as_str()) {
            let _: () = self
                .redis_connection
                .publish(&format!("logs:category:{}", category), &log_json)
                .await?;
        }
        ...
        if let Some(level) = log_data.get("level").and_then(|l| l.as_str()) {
            let _: () = self
                .redis_connection
                .publish(&format!("logs:level:{}", level), &log_json)
                .await?;
        }
...
pub async fn create_agent_logger(
    redis_url: &str,
    service_name: &str,
) -> Result<Box<dyn AgentLogger>> {
