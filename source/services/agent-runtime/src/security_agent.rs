...
use crate::{
    agent::{
        ActivityType, Agent, AgentActivity, AgentInfo, AgentStatus, Capability, CognitivePattern,
        ResourceLimits,
    },
    mrap::{ActionRecord, MrapLoop, MrapState, ReasoningResult, RiskLevel},
};
...
impl SecurityAgent {
    ...
    pub async fn process_query(&mut self, query: &str) -> Result<LogQueryResponse> {
        ...
        details.insert(
            ...
            serde_json::Value::String("log_query".to_string()),
        );
        details.insert(
            "query".to_string(),
            serde_json::Value::String(query.to_string()),
        );
        self.log_activity_with_correlation(ActivityType::ProcessStarted, &details, &correlation_id)
            .await?;
    ...
    async fn execute_mrap_for_query(
        &mut self,
        query: &str,
        correlation_id: &str,
    ) -> Result<MrapState> {
        ...
        mrap_details.insert(
            "cognitive_pattern".to_string(),
            serde_json::Value::String(format!("{:?}", self.info.cognitive_pattern)),
        );
        self.log_activity_with_correlation(
        ...
        completion_details.insert(
            ...
            serde_json::Value::Number(serde_json::Number::from(
                (chrono::Utc::now() - state.started_at).num_milliseconds(),
            )),
        );
        self.log_activity_with_correlation(
            ActivityType::MrapCompleted,
            &completion_details,
            &correlation_id,
        )
        .await?;
...
impl Agent for SecurityAgent {
    ...
    async fn initialize(&mut self) -> Result<()> {
        ...
        info!(
            "Security Agent initialized with cognitive pattern: {:?}",
            self.info.cognitive_pattern
        );
        ...
        init_details.insert(
            ...
            serde_json::Value::Number(serde_json::Number::from(self.info.capabilities.len())),
        );

        if let Err(e) = self
            .log_activity(ActivityType::Initialized, &init_details)
            .await
        {
    ...
    async fn log_activity(
        ...
        info!(
            "SecurityAgent logging activity: {:?} to Redis stream agent:activities",
            activity_type
        );
    ...
    async fn log_activity_with_correlation(
        ...
        let activity_json = serde_json::to_string(&activity)?;
        debug!(
            "SecurityAgent logging correlated activity: {:?} [{}] to Redis stream agent:activities",
            activity_type, correlation_id
        );
        debug!("Activity JSON: {}", activity_json);
...
impl<'a> MrapLoop for SecurityMrapExecutor<'a> {
    async fn monitor(&mut self) -> Result<HashMap<String, serde_json::Value>> {
        ...
        phase_details.insert(
            "phase".to_string(),
            serde_json::Value::String("monitor".to_string()),
        );
        phase_details.insert(
            "query".to_string(),
        ...
        self.agent
            .log_activity_with_correlation(
                ActivityType::MonitorPhase,
                &&phase_details,
                &self.correlation_id,
            )
            .await?;
        ...
        let auth_count: usize = self
            .agent
            .redis
            .llen("logs:category:auth")
            .await
            .unwrap_or(0);
    ...
    async fn reason(
        ...
        decision_details.insert(
            "confidence".to_string(),
            serde_json::Value::Number(
                serde_json::Number::from_f64(reasoning.confidence.into()).unwrap(),
            ),
        );
        decision_details.insert(
            "risk_level".to_string(),
            serde_json::Value::String(format!("{:?}", reasoning.risk_assessment)),
        );
        self.agent
            .log_activity_with_correlation(
    ...
    async fn act(&mut self, _reasoning: &ReasoningResult) -> Result<ActionRecord> {
        ...
        phase_details.insert(
            "action".to_string(),
            serde_json::Value::String("fetch_logs".to_string()),
        );
        self.agent
            .log_activity_with_correlation(
        ...
        result_details.insert(
            "success".to_string(),
            serde_json::Value::Bool(action_record.success),
        );
        result_details.insert(
            "duration_ms".to_string(),
            serde_json::Value::Number(serde_json::Number::from(action_record.duration_ms)),
        );
        result_details.insert(
            "logs_found".to_string(),
            serde_json::Value::Number(serde_json::Number::from(response.total_count)),
        );
        self.agent
            .log_activity_with_correlation(
                ActivityType::ResultRecorded,
                &result_details,
                &self.correlation_id,
            )
            .await?;
        ...
        if let Some(action) = &state.action_taken {
            if action.success {
                ...
                let pattern_key = format!(
                    "query_type_{}",
                    if self.query.contains("error") {
                        "error"
                    } else if self.query.contains("auth") {
                        "auth"
                    } else {
                        "general"
                    }
                ...
                *self
                    ...
                    if let Ok(response) = serde_json::from_value::<LogQueryResponse>(result.clone())
                    {
                        ...
                        if error_count > 10 {
                            insights.push(format!(
                                "Found {} errors - investigation may be needed.",
                                error_count
                            ));
                        }
                    }
                ...
                learning_details.insert(
                    "pattern_learned".to_string(),
                    serde_json::Value::String(pattern_key),
                );
                learning_details.insert(
                    "insights_generated".to_string(),
                    serde_json::Value::Number(serde_json::Number::from(insights.len())),
                );
                learning_details.insert("insights".to_string(), serde_json::json!(insights));
                self.agent
                    .log_activity_with_correlation(
                        ActivityType::Custom("learning".to_string()),
                        &learning_details,
                        &self.correlation_id,
                    )
                    .await?;
