use agent_runtime::{ActivityType, Agent, SecurityAgent};
...
async fn test_security_agent_initialization() {
    ...
    let mut agent = SecurityAgent::new(redis_url.as_str())
        .await
...
async fn test_security_agent_activity_logging() {
    ...
    details.insert(
        "test".to_string(),
        Value::String("mrap_logging".to_string()),
    );

    agent
        .log_activity(ActivityType::Custom("test".to_string()), &details)
        .await
    ...
    for attempt in 1..=5 {
        match redis
            .xread::<&str, &str, redis::streams::StreamReadReply>(&["agent:activities"], &["0"])
            .await
        {
    ...
    assert!(
        stream.ids.len() >= 2,
        "Expected at least 2 activity entries (Initialized + Custom), found {}",
        stream.ids.len()
    );
    ...
    let data = entry
        .map
        .get("data")
        .expect("No data field in stream entry");
    ...
    if let redis::Value::BulkString(bytes) = data {
        ...
        let activity: serde_json::Value =
            serde_json::from_str(&json_str).expect("Invalid JSON in stream data");
...
async fn test_security_agent_mrap_workflow() {
    ...
    assert!(
        stream.ids.len() >= 5,
        "Expected multiple MRAP phase entries, found {}",
        stream.ids.len()
    );
...
async fn test_unified_logging_system() {
    ...
    details.insert(
        "test_type".to_string(),
        Value::String("unified_logging".to_string()),
    );

    agent
        .log_activity(ActivityType::Initialized, &details)
        .await
        .expect("Failed to log activity");

    // Verify activity was logged to agent:activities stream
    let stream_entries: redis::streams::StreamReadReply = redis
        .xread(&["agent:activities"], &["0"])
        .await
        .expect("Failed to read agent:activities stream");
    assert!(
        !stream_entries.keys.is_empty(),
        "No entries found in agent:activities stream"
    );

    // Verify activity was also logged to main logs
    let log_entries: Vec<String> = redis
        .lrange("logs:all", 0, -1)
        .await
        .expect("Failed to read logs:all");
    ...
    let log_data: serde_json::Value =
        serde_json::from_str(latest_log).expect("Invalid JSON in main log");
...
async fn test_agent_trait_implementation() {
    let redis_url =
        std::env::var("TEST_DATABASE_URL").unwrap_or_else(|_| "redis://127.0.0.1:6379".to_string());
    let agent = SecurityAgent::new(redis_url.as_str())
        .await
        .expect("Failed to create Security Agent");
