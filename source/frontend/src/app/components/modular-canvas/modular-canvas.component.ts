...
export class ModularCanvasComponent implements OnInit, AfterViewInit, OnDestroy, CanvasController {
    ...
    try {
      ...
      if ((globalThis as any).__LAYOUT_DEBUG__ && viewNode.layout) {
        try {
          ...
          this.debugSavedLayoutAgainstRaw(result, savedLayoutSnapshot);
        } catch (layoutError) {
          console.warn('[ModularCanvas] Failed to debug layout snapshot', layoutError);
        }
      }
  ...
  private debugSavedLayoutAgainstRaw(rawData: { entities: any[]; relationships: any[] }, layout: CanvasData): void {
    try {
