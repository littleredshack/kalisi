...
class ContainmentRuntimeLayoutEngine {
  ...
  layout(graph, options) {
    ...
    if (runtimeConfig.containmentMode === 'flat') {
      ...
      processedNodes = snapshot.nodes.filter(node => !hiddenByCollapse.has(node.GUID ?? node.id)).map(node => this.layoutContainer(node, layoutMetrics, runtimeConfig, hiddenByCollapse, collapsedNodes));
  ...
  layoutContainer(node, metrics, config, hiddenByCollapse, collapsedNodes) {
