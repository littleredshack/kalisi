...
class ContainmentRuntimeLayoutEngine {
  ...
  layout(graph, options) {
    ...
    if (runtimeConfig.containmentMode === 'flat') {
      ...
      processedNodes = snapshot.nodes.filter(node => !hiddenByCollapse.has(node.GUID ?? node.id)).map(node => this.layoutContainer(node, layoutMetrics, runtimeConfig, hiddenByCollapse, collapsedNodes, nodeConfigManager));
    ...
    const edgesToRender = allEdges.map(edge => {
      ...
      const metadata = {
        ...
        visible: (runtimeConfig.containmentMode === 'flat' || !isContainmentEdge) && !hiddenByCollapse.has(edge.from) && !hiddenByCollapse.has(edge.to)
      };
      return {
        ...edge,
        metadata
  ...
  layoutContainer(node, metrics, config, hiddenByCollapse, collapsedNodes, nodeConfigManager) {
    const guid = node.GUID ?? node.id;
