...
class ContainmentRuntimeLayoutEngine {
  ...
  layoutContainer(node, metrics) {
    ...
    const allSamePosition = laidOutChildren.length > 1 && laidOutChildren.every((child, _, arr) => child.x === arr[0].x && child.y === arr[0].y);
    const childrenHaveExplicitPositions = !allSamePosition && laidOutChildren.every(child => Number.isFinite(child.x) && Number.isFinite(child.y));
    ...
    if (childrenHaveExplicitPositions) {
      console.log(`  Using world-relative positions for ${laidOutChildren.length} children`);
      laidOutChildren.forEach(child => {
        console.log(`    ${child.GUID}: (${child.x}, ${child.y})`);
      });
    }
    if (!childrenHaveExplicitPositions) {
      console.log(`  Applying adaptive grid for ${laidOutChildren.length} children`);
      this.applyAdaptiveGrid(clone, laidOutChildren, metrics);
      laidOutChildren.forEach(child => {
        console.log(`    ${child.GUID}: (${child.x}, ${child.y}) after grid`);
      });
    }
    console.log(`  Before clampChildrenToParent:`);
    laidOutChildren.forEach(child => {
      console.log(`    ${child.GUID}: (${child.x}, ${child.y})`);
    });
    this.clampChildrenToParent(clone, laidOutChildren, metrics);
    console.log(`  After clampChildrenToParent:`);
    laidOutChildren.forEach(child => {
      console.log(`    ${child.GUID}: (${child.x}, ${child.y})`);
    });
  ...
  applyAdaptiveGrid(parent, children, metrics) {
    ...
    _canvas_layout_primitives__WEBPACK_IMPORTED_MODULE_2__.LayoutPrimitives.resizeToFitChildren(parent, padding, padding);
  }
  ...
  computeEdgeWaypoints(nodes, edges) {
...
class ContainmentGridLayoutEngine {
  ...
  layoutContainer(node) {
    ...
    _canvas_layout_primitives__WEBPACK_IMPORTED_MODULE_1__.LayoutPrimitives.calculateGridPositions(node.children, node.width - CONTAINER_PADDING, node.height - CONTAINER_PADDING, CONTAINER_PADDING / 2, CONTAINER_PADDING / 2, CHILD_SPACING);
