...
/***/ 1581:
/*!*****************************************************************!*\
  !*** ./src/app/shared/layouts/engines/force-directed-layout.ts ***!
  \*****************************************************************/
/***/ ((__unused_webpack_module, __webpack_exports__, __webpack_require__) => {

__webpack_require__.r(__webpack_exports__);
/* harmony export */ __webpack_require__.d(__webpack_exports__, {
/* harmony export */   applyForceDirectedLayout: () => (/* binding */ applyForceDirectedLayout)
/* harmony export */ });
/**
 ...
 */
function applyForceDirectedLayout(nodes, edges, options = {}) {
  const opts = {
    ...DEFAULT_OPTIONS,
    ...options
  };
...
/* harmony export */ });
/* harmony import */ var _engines_containment_runtime_layout_engine__WEBPACK_IMPORTED_MODULE_0__ = __webpack_require__(/*! ./engines/containment-runtime-layout.engine */ 5456);
/* harmony import */ var _engines_force_directed_layout_engine__WEBPACK_IMPORTED_MODULE_1__ = __webpack_require__(/*! ./engines/force-directed-layout.engine */ 9733);


function registerDefaultLayoutEngines(orchestrator) {
  orchestrator.registerEngine(new _engines_containment_runtime_layout_engine__WEBPACK_IMPORTED_MODULE_0__.ContainmentRuntimeLayoutEngine());
  orchestrator.registerEngine(new _engines_force_directed_layout_engine__WEBPACK_IMPORTED_MODULE_1__.ForceDirectedLayoutEngine());
  return orchestrator;
}
...
function createNodeFromLayout(layoutNode) {
  const node = {
    ...
    dragging: layoutNode.metadata['dragging'] ?? false,
    metadata: {
      ...layoutNode.metadata
    }
  };
  ...
  const visit = (node, depth = 0) => {
    ...
    nodesRecord[nodeId] = {
      ...
      metadata: {
        ...
        _userLocked: node._userLocked ?? false
      },
...
function canvasDataToLayoutGraph(data, layoutVersion = 1) {
  ...
  const snapshot = {
    nodes: data.nodes,
    // Direct reference
    edges: data.originalEdges ?? data.edges,
    // Direct reference
    metadata: {
...
/***/ 4982:
/*!**************************************************************!*\
  !*** ./src/app/shared/layouts/helpers/flat-layout-helper.ts ***!
  \**************************************************************/
/***/ ((__unused_webpack_module, __webpack_exports__, __webpack_require__) => {

__webpack_require__.r(__webpack_exports__);
/* harmony export */ __webpack_require__.d(__webpack_exports__, {
/* harmony export */   applyFlatGridLayout: () => (/* binding */ applyFlatGridLayout),
/* harmony export */   flattenHierarchyWithEdges: () => (/* binding */ flattenHierarchyWithEdges),
/* harmony export */   setAbsoluteWorldPositions: () => (/* binding */ setAbsoluteWorldPositions)
/* harmony export */ });
/**
 * Flatten hierarchy and generate CONTAINS edges
 */
function flattenHierarchyWithEdges(hierarchyRoots, hiddenByCollapse) {
  const flatNodes = [];
  const containsEdges = [];
 ...
 */
function applyFlatGridLayout(nodes, metrics) {
  if (nodes.length === 0) {
    return;
  }
  const {
 ...
 */
function setAbsoluteWorldPositions(nodes) {
  nodes.forEach(node => {
    node.metadata = {
      ...(node.metadata ?? {}),
      worldPosition: {
...
/* harmony import */ var _helpers_flat_layout_helper__WEBPACK_IMPORTED_MODULE_3__ = __webpack_require__(/*! ../helpers/flat-layout-helper */ 4982);
...
class ContainmentRuntimeLayoutEngine {
  ...
  layout(graph, options) {
    ...
    const nodeConfigManager = options.engineOptions?.['nodeConfigManager'];
    const layoutMetrics = {
    ...
    if (runtimeConfig.containmentMode === 'flat') {
      ...
      const perNodeContainsEdges = [];
      processedNodes = snapshot.nodes.filter(node => !hiddenByCollapse.has(node.GUID ?? node.id)).map(node => this.layoutContainer(node, layoutMetrics, runtimeConfig, hiddenByCollapse, collapsedNodes, nodeConfigManager, perNodeContainsEdges));
      processedNodes.forEach(root => this.updateWorldMetadata(root));
      rootIds = processedNodes.map(node => node.GUID ?? node.id).filter(value => Boolean(value));
      generatedContainsEdges = perNodeContainsEdges;
    }
    // Collect generated edges from flattened nodes' metadata
    const collectGeneratedEdges = nodes => {
      const collected = [];
      nodes.forEach(node => {
        const flattenedEdges = node.metadata?.['flattenedEdges'];
        if (flattenedEdges) {
    ...
    const edgesToRender = allEdges.map(edge => {
      ...
      const relationTypeSource = typeof existingMetadata['relationType'] === 'string' ? existingMetadata['relationType'] : typeof edge.label === 'string' && edge.label.length > 0 ? edge.label : '';
      ...
      const metadata = {
        ...
        visible: (shouldShowContainsEdge || !isContainmentEdge) && !hiddenByCollapse.has(edge.fromGUID) && !hiddenByCollapse.has(edge.toGUID)
      };
      return {
        ...edge,
        metadata
    ...
    const resolvedRootIds = rootIds && rootIds.length > 0 ? Array.from(new Set(rootIds)) : processedNodes.map(node => node.GUID ?? node.id).filter(value => Boolean(value));
  ...
  layoutContainer(node, metrics, config, hiddenByCollapse, collapsedNodes, nodeConfigManager, containsEdgeCollector) {
    ...
    if (effectiveConfig.containmentMode === 'flat') {
      ...
      return this.layoutNodeAsHierarchical(node, visibleChildren, metrics, config, hiddenByCollapse, collapsedNodes, nodeConfigManager, containsEdgeCollector, effectiveConfig);
    }
  }
  /**
   * Resolve effective configuration for a node (global + per-node overrides)
   */
  resolveEffectiveConfiguration(node, globalConfig, nodeConfigManager) {
    ...
    if (nodeConfigManager) {
      ...
      if (guid) {
        ...
        if (resolved.layoutStrategy !== 'manual') {
          layoutMode = resolved.layoutStrategy;
        }
      }
    }
    return {
   ...
   */
  layoutNodeAsFlat(node, visibleChildren, metrics, hiddenByCollapse, containsEdgeCollector) {
    // Flatten hierarchy and extract CONTAINS edges
    const flatResult = (0,_helpers_flat_layout_helper__WEBPACK_IMPORTED_MODULE_3__.flattenHierarchyWithEdges)(visibleChildren, hiddenByCollapse);
    ...
    flatResult.nodes.forEach(flatNode => {
      ...
      const minSize = _canvas_layout_primitives__WEBPACK_IMPORTED_MODULE_2__.LayoutPrimitives.getMinimumNodeSize(flatNode.type);
    ...
    const headerOffset = _canvas_layout_primitives__WEBPACK_IMPORTED_MODULE_2__.LayoutPrimitives.computeHeaderOffset(node);
    ...
    const result = {
      ...
      // ← Canonical flat structure with custom positions
      width: requiredWidth,
      height: requiredHeight,
      metadata: {
        ...(node.metadata ?? {}),
        perNodeFlattened: true,
        flattenedChildren: flatResult.nodes,
        // ← References to same objects (for renderer check)
   ...
   */
  layoutNodeAsHierarchical(node, visibleChildren, metrics, globalConfig, hiddenByCollapse, collapsedNodes, nodeConfigManager, containsEdgeCollector, effectiveConfig) {
    // Recursively layout children first
    const laidOutChildren = visibleChildren.map(child => this.layoutContainer(child, metrics, globalConfig, hiddenByCollapse, collapsedNodes, nodeConfigManager, containsEdgeCollector));
    ...
    _canvas_layout_primitives__WEBPACK_IMPORTED_MODULE_2__.LayoutPrimitives.resizeToFitChildren(result, metrics.padding, metrics.padding);
    return this.ensureDefaults(result);
  }
  /**
   * Apply layout strategy to position children (strategy pattern entry point)
   */
  applyLayoutStrategy(parent, children, metrics, strategy) {
    switch (strategy) {
      case 'grid':
   ...
   */
  applyGridLayoutToNodes(nodes, metrics) {
  ...
  addInheritedEdges(edges, hierarchyRoots, hiddenByCollapse) {
  ...
  buildVisibilityMap(roots, hiddenByCollapse) {
    ...
    const resolve = id => {
      ...
      const info = {
        visible: false,
        visibleAncestor: parentInfo.visibleAncestor
      };
      cache.set(id, info);
      return info;
    };
  ...
  computeEdgeWaypoints(nodes, edges, config) {
    ...
    const isFlat = config.containmentMode === 'flat' || nodes.every(node => !node.children || node.children.length === 0);
   ...
   */
  layoutFlatFromHierarchy(hierarchyRoots, metrics, config, hiddenByCollapse) {
    // Flatten hierarchy (but don't generate edges - they already exist in original data)
    const flatResult = (0,_helpers_flat_layout_helper__WEBPACK_IMPORTED_MODULE_3__.flattenHierarchyWithEdges)(hierarchyRoots, hiddenByCollapse);
    ...
    (0,_helpers_flat_layout_helper__WEBPACK_IMPORTED_MODULE_3__.applyFlatGridLayout)(flatResult.nodes, {
      gap: metrics.gap,
      padding: metrics.padding
    });
    // Set absolute world positions
    (0,_helpers_flat_layout_helper__WEBPACK_IMPORTED_MODULE_3__.setAbsoluteWorldPositions)(flatResult.nodes);
    const rootIds = flatResult.nodes.map(node => node.GUID ?? node.id).filter(value => Boolean(value));
    return {
...
/***/ 9733:
/*!************************************************************************!*\
  !*** ./src/app/shared/layouts/engines/force-directed-layout.engine.ts ***!
  \************************************************************************/
/***/ ((__unused_webpack_module, __webpack_exports__, __webpack_require__) => {

__webpack_require__.r(__webpack_exports__);
/* harmony export */ __webpack_require__.d(__webpack_exports__, {
/* harmony export */   ForceDirectedLayoutEngine: () => (/* binding */ ForceDirectedLayoutEngine)
/* harmony export */ });
/* harmony import */ var _force_directed_layout__WEBPACK_IMPORTED_MODULE_0__ = __webpack_require__(/*! ./force-directed-layout */ 1581);
...
class ForceDirectedLayoutEngine {
  ...
  layout(graph, options) {
    ...
    (0,_force_directed_layout__WEBPACK_IMPORTED_MODULE_0__.applyForceDirectedLayout)(nodes, edges, {
      width: 800,
      height: 600,
      iterations: 100
    });
