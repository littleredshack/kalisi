...
pub async fn logging_middleware(
    ...
    request_context.insert("request_id".to_string(), serde_json::json!(request_id.clone()));
    ...
    state.logger.log_with_context(
        level,
        LogCategory::Api,
        &format!("{} {} - {} in {}ms", method, path, status.as_u16(), duration_ms),
        response_context,
    ).await;
...
pub async fn error_logging_middleware(
    ...
    if response.status().is_server_error() {
        ...
        context.insert("status".to_string(), serde_json::json!(response.status().as_u16()));
