...
impl CentralLogger {
    ...
    pub async fn log(&self, level: LogLevel, category: LogCategory, message: &str) {
        self.log_with_context(level, category, message, HashMap::new()).await;
    }
    ...
    pub async fn log_with_context(
        ...
        let entry = LogEntry {
            ...
            data: if context.is_empty() { None } else { Some(context) },
        };
        ...
        if let Ok(json) = serde_json::to_string(&entry) {
            ...
            let category_key = format!("logs:category:{}", format!("{:?}", category).to_lowercase());
            ...
            let _: Result<(), _> = redis.publish(&format!("logs:category:{}", format!("{:?}", category).to_lowercase()), &notification_json).await;
            let _: Result<(), _> = redis.publish(&format!("logs:level:{}", format!("{:?}", level).to_lowercase()), &notification_json).await;
    ...
    pub async fn get_logs(
        ...
        // Get logs from Redis
        let entries: Vec<String> = redis.lrange(&key, 0, (limit - 1) as isize)
            .await
            .unwrap_or_default();
    ...
    pub async fn clear_old_logs(&self, _days: i64) {
        ...
        // Clear category logs
        for category in ["auth", "api", "chat", "websocket", "system", "security", "error", "performance"] {
    ...
    pub async fn info(&self, category: LogCategory, message: &str, context: HashMap<String, Value>) {
        self.log_with_context(LogLevel::Info, category, message, context).await;
    }
    
    pub async fn error(&self, category: LogCategory, message: &str, context: HashMap<String, Value>) {
        self.log_with_context(LogLevel::Error, category, message, context).await;
    }
    
    // Backward compatibility without context
    #[allow(dead_code)]
    pub async fn info_simple(&self, category: LogCategory, message: &str) {
        self.log_with_context(LogLevel::Info, category, message, HashMap::new()).await;
    }
    
    #[allow(dead_code)]
    pub async fn error_simple(&self, category: LogCategory, message: &str) {
        self.log_with_context(LogLevel::Error, category, message, HashMap::new()).await;
    }
    
    pub async fn log_security_event(&self, event: impl std::any::Any) {
        // Try to downcast to SecurityEvent
        if let Some(sec_event) = (&event as &dyn std::any::Any).downcast_ref::<security_events::SecurityEvent>() {
            let mut context = HashMap::new();
            context.insert("event_type".to_string(), serde_json::json!(format!("{:?}", sec_event.event_type)));
            context.insert("severity".to_string(), serde_json::json!(format!("{:?}", sec_event.severity)));
            ...
            self.log(LogLevel::Info, LogCategory::Security, "Security event").await;
        }
    }
    ...
    pub async fn log_login_attempt(&self, email: &str, ip: Option<String>, success: bool) {
        ...
        self.log_with_context(level, LogCategory::Auth, &message, context).await;
    }
