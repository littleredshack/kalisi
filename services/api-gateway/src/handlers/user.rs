...
use crate::{
    ...
    storage::{UserStorage, SessionStorage},
...
pub async fn update_profile(
    ...
    get_profile(State(state), Extension(user)).await.into_response()
}
...
pub async fn get_account_info(
    ...
    let user_data = match user_storage.get_user_by_id(user.user_id).await {
        ...
        Err(e) => {
            ...
            return (
                StatusCode::INTERNAL_SERVER_ERROR,
                Json(ApiResponse::<()>::error("Failed to retrieve account information")),
            ).into_response();
        }
    };
...
pub async fn delete_account(
    ...
    ).with_details(format!("Account deletion initiated for user: {}", user.email))
     .with_severity(SecuritySeverity::High)).await;
    ...
    match user_storage.delete_user_by_id(user.user_id).await {
        Ok(_) => {
            ...
            state.logger.log_security_event(SecurityEvent::new(
                ...
                Some(user.email.clone()),
            ).with_details(format!("Account successfully deleted for user: {}", user.email))
             .with_severity(SecuritySeverity::High)).await;
            
            // Send confirmation email
            let user_name = user.email.split('@').next().unwrap_or(&user.email);
            if let Err(e) = state.email_service.send_account_deletion_confirmation(&user.email, user_name).await {
                error!("Failed to send account deletion confirmation email to {}: {}", user.email, e);
            } else {
            ...
            let response = DeleteAccountResponse {
                success: true,
                message: "Your account has been successfully deleted. All data has been removed.".to_string(),
            };
        ...
        Err(e) => {
            ...
            (
                StatusCode::INTERNAL_SERVER_ERROR,
                Json(ApiResponse::<()>::error("Failed to delete account. Please try again later.")),
            ).into_response()
        }
    }
}
