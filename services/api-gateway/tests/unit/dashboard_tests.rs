...
async fn test_dashboard_api_stats() {
    ...
    // Create test data
    {
        ...
        let user_id = storage.create_user("stats@example.com", "Stats User")
            .await
            .expect("Failed to create user");
        
        // Log various auth events
        storage.log_auth_event(user_id, "login", true, Some("127.0.0.1".to_string()), None)
            .await
            .expect("Failed to log event");
        storage.log_auth_event(user_id, "login", false, Some("127.0.0.1".to_string()), None)
            .await
            .expect("Failed to log event");
    }
    ...
    let app = test::init_service(
        App::new()
            .app_data(state.clone())
            .service(web::resource("/api/dashboard/stats").route(web::get().to(get_dashboard_stats)))
    ).await;
...
async fn test_dashboard_recent_activity() {
    ...
    {
        let mut storage = state.storage.write().await;
        let user_id = storage.create_user("activity@example.com", "Activity User")
            .await
            .expect("Failed to create user");
        
        storage.create_session(user_id, "test-session")
            .await
            .expect("Failed to create session");
            
        storage.log_auth_event(user_id, "login", true, Some("192.168.1.1".to_string()), Some("Test Browser".to_string()))
            .await
            .expect("Failed to log event");
    }
    ...
    let app = test::init_service(
        App::new()
            .app_data(state.clone())
            .service(web::resource("/api/dashboard/activity").route(web::get().to(get_recent_activity)))
    ).await;
...
async fn test_dashboard_system_health() {
    ...
    let app = test::init_service(
        App::new()
            .app_data(state.clone())
            .service(web::resource("/api/dashboard/health").route(web::get().to(get_system_health)))
    ).await;
...
async fn test_dashboard_user_sessions_list() {
    ...
    {
        let mut storage = state.storage.write().await;
        let user_id = storage.create_user("sessions@example.com", "Session User")
            .await
            .expect("Failed to create user");
        
        for i in 0..3 {
            storage.create_session(user_id, &format!("session-{}", i))
                .await
                .expect("Failed to create session");
        }
    }
    ...
    let app = test::init_service(
        App::new()
            .app_data(state.clone())
            .service(web::resource("/api/dashboard/sessions").route(web::get().to(get_active_sessions)))
    ).await;
