...
use edt_api_gateway::middleware::security_headers::{
    security_headers_middleware, CspNonce, csp_report_handler
};
...
async fn test_csp_nonce_generation() {
    let app = Router::new()
        .route("/test", get(|request: Request<Body>| async move {
...
async fn test_csp_nonce_uniqueness() {
    ...
    for _ in 0..5 {
        ...
        assert!(!nonces.contains(&nonce), "Nonce should be unique: {}", nonce);
        nonces.push(nonce);
    }
}
...
async fn test_csp_hardened_directives() {
    ...
    assert!(!csp.contains("unsafe-inline"), "CSP should not contain unsafe-inline");
    assert!(!csp.contains("unsafe-eval"), "CSP should not contain unsafe-eval");
    
    // Should use nonces instead
    assert!(csp.contains("nonce-"), "CSP should use nonces instead of unsafe directives");
}
...
async fn test_permissions_policy_restrictive() {
    ...
    let response = app.oneshot(request).await.unwrap();
    let permissions = response.headers()
    ...
    for feature in &sensitive_features {
        assert!(permissions.contains(feature), 
...
async fn test_csp_violation_reporting() {
    ...
    let app = Router::new()
        .route("/csp-report", axum::routing::post(csp_report_handler));
