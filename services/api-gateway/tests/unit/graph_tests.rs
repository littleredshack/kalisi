...
async fn test_create_and_get_node() {
    ...
    let node = client.create_node("TestLabel", properties.clone())
        .await
    ...
    // Get the node back
    let retrieved = client.get_node(node.id)
        .await
...
async fn test_create_relationship() {
    ...
    // Create two nodes
    let node1 = client.create_node("Person", json!({"name": "Alice"}))
        .await
        .expect("Failed to create node 1");
    
    let node2 = client.create_node("Person", json!({"name": "Bob"}))
        .await
...
async fn test_update_node() {
    ...
    // Create a node
    let original = client.create_node("Document", json!({"title": "Original Title"}))
        .await
        .expect("Failed to create node");
    ...
    let updated = client.update_node(original.id, updated_props)
        .await
...
async fn test_delete_node() {
    ...
    // Create a node
    let node = client.create_node("Temporary", json!({"temp": true}))
        .await
        .expect("Failed to create node");
...
async fn test_find_nodes_by_label() {
    ...
    for i in 0..3 {
        client.create_node("TestGroup", json!({"index": i}))
            .await
            .expect("Failed to create node");
    }
    
    // Find them
    let nodes = client.find_nodes_by_label("TestGroup")
        .await
        .expect("Failed to find nodes");
    
    assert!(nodes.len() >= 3);
    assert!(nodes.iter().all(|n| n.labels.contains(&"TestGroup".to_string())));
}
...
async fn test_execute_cypher_query() {
    ...
    let mut result = client.client.execute(
        neo4rs::query(result_query)
    )
    .await
...
async fn test_shortest_path() {
    ...
    let mut result = client.client.execute(neo4rs::query(query))
        .await
    ...
    let mut path_result = client.client.execute(
...
async fn test_transaction_rollback() {
    ...
    // Start a transaction
    let txn = client.client.start_txn().await.expect("Failed to start transaction");
    ...
    let mut result = client.client.execute(neo4rs::query(check_query))
        .await
...
async fn test_complex_graph_pattern() {
    ...
    client.execute_query(setup_query)
        .await
    ...
    let mut result = client.client.execute(neo4rs::query(common_interests_query))
        .await
    ...
    assert!(connections.iter().any(|(p1, p2, i)| 
        p1 == "Alice" && p2 == "Bob" && i == "Technology"
    ));
    // Alice and Dave both like Music
    assert!(connections.iter().any(|(p1, p2, i)| 
        p1 == "Alice" && p2 == "Dave" && i == "Music"
    ));
}