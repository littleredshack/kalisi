...
mod tests {
    ...
    async fn setup_test_env() -> TestContext {
        ...
        std::env::set_var("JWT_SECRET", "test-jwt-secret-key-for-testing-must-be-long-enough");
    ...
    async fn test_v2_mfa_status_without_auth() {
        ...
        let response = context.request("GET", "/v2/auth/mfa/status", None, None).await;
...
mod integration_tests {
    ...
    async fn test_full_auth_flow_performance() {
        ...
        assert!(duration.as_millis() < 2000, "Request should complete within 2 seconds");
    ...
    async fn test_environment_configuration() {
        ...
        assert!(context.config.approved_emails.contains(&"test@example.com".to_string()));
    }
}