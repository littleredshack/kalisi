...
impl RedisBus {
    ...
    pub async fn publish(&mut self, envelope: &Envelope, payload: &serde_json::Value) -> Result<String> {
    ...
    pub async fn subscribe(&mut self, agent_id: &str, last_id: &str) -> Result<Vec<(Envelope, serde_json::Value)>> {
        let stream_key = format!("{}{}", self.stream_prefix, agent_id);
        
        // Read from stream
        let result: streams::StreamReadReply = self.redis
            .xread_options(
                &[&stream_key],
                &[last_id],
    ...
    pub async fn request(&mut self, envelope: &Envelope, payload: &serde_json::Value, timeout_ms: u64) -> Result<(Envelope, serde_json::Value)> {
                        ...
                        if let Some(payload_str) = message.map.get("payload") {
                            if let redis::Value::BulkString(envelope_bytes) = envelope_str {
                                if let redis::Value::BulkString(payload_bytes) = payload_str {
                                    ...
                                    if let Ok(response_envelope) = serde_json::from_str::<Envelope>(&envelope_json) {
                                        if let Ok(response_payload) = serde_json::from_str::<serde_json::Value>(&payload_json) {
    ...
    pub async fn register_agent(&mut self, agent_id: &str, capabilities: Vec<String>) -> Result<()> {
        ...
        self.redis.set::<_, _, ()>(&key, serde_json::to_string(&data)?).await?;
